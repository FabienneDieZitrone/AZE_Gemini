...
if (isset($get_params['code'])) {
    try {
        ...
        // CRITICAL FIX: User ID aus Datenbank holen und in Session speichern
        require_once __DIR__ . '/db.php';
        $stmt = $conn->prepare("SELECT id, role, email FROM users WHERE oid = ?");
        $stmt->bind_param("s", $id_token_payload['oid']);
        $stmt->execute();
        $result = $stmt->get_result();
        
        if ($user = $result->fetch_assoc()) {
            // User exists - add database ID to session
            ...
            $stmt->close(); // Close SELECT statement first
            
            $stmt = $conn->prepare("INSERT INTO users (oid, username, name, email, role) VALUES (?, ?, ?, ?, 'employee')");
